...
do  
    mkdir -p test_circuits/2^${i}
    for j in $(seq ${LOW_LOOKUP_SIZE} ${HIGH_LOOKUP_SIZE})
    do
        # Generate a new build/2^{i} folder
        nargo new test_circuits/2^${i}/2^${j}
        pushd test_circuits/2^${i}/2^${j}

        # replace {{N}} in src/main.nr    
        cat ../../../src/main.nr | STR_SIZE=$((2**$i)) LOOKUP_SIZE=$((2**$j)) envsubst > ./src/main.nr
